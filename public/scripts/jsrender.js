...
function View( context1, path, parentView, data, template, index ) {
...
function renderContent( data, context1, parentView, path, index ) {
	...
	var i, l, dataItem, newView, itemWrap, itemsWrap, itemResult, parentcontext1, tmpl, layout,
		props = {},
	...
	// Set additional context1 on views created here, (as modified context1 inherited from the parent, and be inherited by child views)
	// Note: If no jQuery, extend does not support chained copies - so limit extend() to two parameters
	context1 = (context1 && context1 === parentcontext1)
		? parentcontext1
		: (parentcontext1
			// if parentcontext1, make copy
			? ((parentcontext1 = extend( {}, parentcontext1 ), context1)
				// If context1, merge context1 with copied parentcontext1
				? extend( parentcontext1, context1 )
				: parentcontext1)
			// if no parentcontext1, use context1, or default to {}
			: context1 || {});
	...
	itemWrap = context1.link && sub.onRenderItem;
	itemsWrap = context1.link && sub.onRenderItems;
	...
	if ( tmpl ) {
		if ( $.isArray( data ) && !layout ) {
			...
			newView = swapContent ? parentView : (index !== undefined && parentView) || View( context1, path, parentView, data, tmpl, index );

			for ( i = 0, l = data.length; i < l; i++ ) {
				// Create a view for each data item.
				dataItem = data[ i ];
				itemResult = tmpl.fn( dataItem, View( context1, path, newView, dataItem, tmpl, (index||0) + i ), jsv );
				result += itemWrap ? itemWrap( itemResult, props ) : itemResult;
			}
		} else {
			// Create a view for singleton data object.
			newView = swapContent ? parentView : View( context1, path, parentView, data, tmpl, index );
...
tags({
	"if": function() {
		...
		view.onElse = function( tagObject, args ) {
			...
			// Test is satisfied, so render content, while remaining in current data context1
			// By passing the view, we inherit data context1 from the parent view, and the content is treated as a layout template
